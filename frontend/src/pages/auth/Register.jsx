...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      setErrors({ _global: error.message || t('auth.registerFailed') });
      return;
    }

    // If email confirmation is enabled in Supabase (Auth → Settings), the
    // session comes back null and the user needs to click a link before
    // they can sign in. Otherwise the session is already active.
    if (!data.session) {
      setConfirmSent(true);
      return;
    }
